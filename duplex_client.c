...
// full duplex client implementation
...
int main(int argc, char** argv)
{
        ...
        if (argc != 2) {
                fprintf(stderr, "usage: client hostnamr\n");
                exit(1);
        }
        ...
        while (1) {
                ...
                if (FD_ISSET(sockfd, &copy)) {
                        if ((numbytes = recvfrom(sockfd, msg, MAXBUFLEN-1, 0, (struct sockaddr *)&my_addr, &addr_len)) == 1 && errno != 0) {
                                perror("recv");
                                exit(1);
                        }
